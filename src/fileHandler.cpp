...
int fileHandler::loadCandidates(Candidate*& candidates, const std::string& filename) {
    ...
    while (in >> id >> name >> party >> votes) {
        if (count >= capacity) {
            ...
            for (int i = 0; i < count; ++i) temp[i] = candidates[i];
            delete[] candidates;
            candidates = temp;
        }
        candidates[count] = Candidate(name, party, id);
        for (int i = 0; i < votes; ++i) candidates[count].incrementVotes();
        ++count;
    }
    in.close();
    return count;
...
void fileHandler::saveElections(Election** elections, int electionCount, const std::string& filename) {
    ...
    for (int i = 0; i < electionCount; ++i) {
        // Save type, title, status, endTime, candidateCount, city/country
...
int fileHandler::loadElections(Election**& elections, const std::string& filename) {
    ...
    while (in >> type >> title >> status >> endTime) {
        ...
        if (count >= capacity) {
            ...
            for (int i = 0; i < count; ++i) temp[i] = elections[i];
            delete[] elections;
            elections = temp;
        }
        Election* e = nullptr;
        ...
        for (int j = 0; j < candidateCount; ++j) {
            ...
            c->setId(cid);
            for (int v = 0; v < votes; ++v) c->incrementVotes();
        }
        elections[count++] = e;
    }
    in.close();
...
int fileHandler::loadUsers(User**& users, const std::string& filename) {
    ...
    while (in >> id >> username >> password) {
        if (count >= capacity) {
            ...
            for (int i = 0; i < count; ++i) temp[i] = users[i];
            delete[] users;
            users = temp;
        }
        users[count++] = new Voter(username, password, id); // Default to Voter for demo
    }
    in.close();
    return count;
}
